...
from src.data.treebank import TreeBankDataset
from src.utils.tensorboard import Tensorboard
from src.models.pretrain.bert import BERTModel
from src.models.finetune.text_tagging import BertTextTagging
from src.models.losses.loss_text_tagging import TextTaggingLoss
...
class Trainer:
    ...
    def create_data_loader(self):
        self.train_dataset = TreeBankDataset(self.config, data_type='train')
        self.valid_dataset = TreeBankDataset(self.config, data_type='valid')
    ...
    def create_model(self):
        self.model = BertTextTagging(self.pretrained_model, self.config['n_classes']).to(self.config['device'])
        self.optimizer = torch.optim.AdamW(params=self.model.parameters(), lr=self.config['lr'])
        self.loss_fn = TextTaggingLoss()
